...
JOIN Booking_Seats bs ON b.booking_id = bs.booking_id AND bu.user_id = bs.user_id
JOIN Show_Seats ss ON bs.show_seat_id = ss.show_seat_id
JOIN Seats st ON ss.seat_id = st.seat_id
WHERE bu.user_id = 1
  AND s.date BETWEEN '2026-04-01' AND '2026-05-30'
GROUP BY b.booking_id, m.title, t.name, s.start_time;
